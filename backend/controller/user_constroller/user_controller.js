const { users_model, Users_model_otp } = require("../../models/db_model/db_model");
...
const verification_user_by_otp = async (req, res) => {
  try {
    ...
    if(!find_user_with_OTP) error_res(res,{status_code: 400,message: "otp not valid"})
      
    const reslut_verification = await jwt.verify(find_user_with_OTP.token, json_screat);
